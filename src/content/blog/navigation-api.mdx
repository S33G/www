...
The key detail: [`event.formData`](https://developer.mozilla.org/en-US/docs/Web/API/NavigateEvent/formData) is `null` for link clicks and non-null for form submissions. That's how you route between GET and POST handling. A plain `<form action="/search" method="post">` gets intercepted like any other navigation — no wrappers, no manual `onSubmit` handlers.
...
The ~~[aleph.js](https://github.com/alephjs/aleph.js)~~ [Fresh](https://fresh.deno.dev/) framework already ships this pattern in production.
