...
Once I knew _what_ to build, I needed a plan. I used Opencode (specifically the Sisyphus agent) to break the project down into actionable tasks. We defined the architecture, the component structure, and the data models.
...
The conversion logic needs to be rock solid. We decided to normalize everything to a "base unit" first, then convert to the target unit. This avoids defining conversion factors for every possible pair—with n units, that would be n² definitions instead of just n.
